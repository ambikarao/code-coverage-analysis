...
Execute appropriate linting tools based on the project (e.g., ESLint for JavaScript/TypeScript, pylint for Python, etc.) using the shell command `npm run lint || python -m pylint src/ || echo "No linting configured"`. Analyze the output to identify code style violations, formatting issues, and adherence to coding standards. Store the linting results as `linting_findings`.

## Step 5: Ask User for Approval to Apply Fixes
...
## Step 6: Generate Optimization Patches (Conditional)
...
## Step 9 (Conditional): Create Git Branch
...
## Step 10 (Conditional): Stage Changes
...
## Step 11 (Conditional): Commit Changes
...
## Step 12 (Conditional): Push Changes
