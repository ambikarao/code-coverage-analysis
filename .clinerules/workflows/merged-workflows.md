...
These instructions describe performing lint checks by detecting project technology and applying patches for the issues.
...
Use AI agent to:
Parse lint issues.
For each issue, prioritise based on severity and rule ID (e.g., S1854 for unused variables, S6479 for index keys, S6481 for performance).
Apply minimal fixes safely preserving behavior.
Generate unified diff patches for changes.
